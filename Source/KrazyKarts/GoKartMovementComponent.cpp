...
void UGoKartMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	// 클라이언트의 역할이 AutonomousProxy 일때, 서버에서 서버 제어 차량의 원격 역할이 simulatedProxy 일떄???
...
{
	// GetOwner()로 해당 컴포넌트를 가지고 있는 액터를 가져와서 수행.
	FVector Force = GetOwner()->GetActorForwardVector() * MaxDrivingForce * Move.Throttle;

	Force += GetAirResistance() + GetRollingResistance();

	FVector Accelation = Force / Mass;

	Velocity += Move.DeltaTime * Accelation;
